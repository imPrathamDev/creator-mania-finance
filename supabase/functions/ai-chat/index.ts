...
You help users understand their financial data by calling the available functions to fetch real data.
...
function resolvePeriod(
  ...
  switch (period) {
    ...
    case "last_year":
      return { from: fmt(ymd(y - 1, 0, 1)), to: fmt(ymd(y - 1, 11, 31)) };
    case "last_7_days": {
...
async function getPending() {
  ...
  const out = {
    ...
    oldest_overdue: null as string | null,
  };
  for (const r of data ?? []) {
    const amt = Number(r.amount);
    ...
    if (r.due_date) {
      ...
      if (
        ...
        (!out.oldest_overdue || r.due_date < out.oldest_overdue)
      )
...
async function getTagBreakdown(
  ...
  for (const r of data ?? []) {
    ...
    const list = tags.length
      ? tags
      : [{ id: "__untagged__", name: "Untagged", color: null }];
...
async function searchTransactions(
  ...
  let q = supabase
    .from("transactions")
    .select(
      "id, title, type, amount, currency, payment_status, transaction_date, contact:contacts(name)",
    )
    .or(
      `title.ilike.%${query}%,notes.ilike.%${query}%,description.ilike.%${query}%,reference_number.ilike.%${query}%`,
    )
    .limit(limit);
...
const PERIOD_PARAM = {
  ...
  enum: PERIOD_ENUM,
  description:
    "Time period. Use 'custom' with date_from/date_to for a custom range.",
};
const DATE_FROM = {
  type: "string",
  description: "Start date YYYY-MM-DD (only for custom period)",
};
const DATE_TO = {
  type: "string",
  description: "End date YYYY-MM-DD (only for custom period)",
};
...
const TOOLS: OpenAI.ChatCompletionTool[] = [
  {
    ...
    function: {
      name: "get_summary",
      description:
        "Get total income, expense, net, counts, and averages for a period.",
      parameters: {
        type: "object",
        properties: {
          period: PERIOD_PARAM,
  ...
  },
  {
    ...
    function: {
      name: "get_comparison",
      description:
        "Compare current period vs previous period — income, expense, net % changes.",
      parameters: {
        type: "object",
        properties: {
          period: PERIOD_PARAM,
  ...
  },
  {
    ...
    function: {
      name: "get_pending",
      description:
        "Get all pending, overdue, and partially paid transaction counts, amounts, and upcoming dues.",
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get spending and income broken down by tag/category for a period.",
      parameters: {
        type: "object",
        properties: {
          period: PERIOD_PARAM,
  ...
  },
  {
    ...
    function: {
      ...
      description: "Get top clients/vendors by total transaction amount.",
      parameters: {
        type: "object",
        properties: {
          period: PERIOD_PARAM,
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get the largest income or expense transactions for a period.",
      parameters: {
        type: "object",
        properties: {
          period: PERIOD_PARAM,
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Get breakdown by payment method (UPI, cash, card, etc.) with income and expense split.",
      parameters: {
        type: "object",
        properties: {
          period: PERIOD_PARAM,
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Search transactions by title, notes, or reference number. Optionally filter by type or status.",
      parameters: {
        type: "object",
        properties: {
          query: { type: "string" },
...
async function executeTool(name: string, args: any): Promise<any> {
  switch (name) {
    case "get_summary":
      return getSummary(args.period, args.date_from, args.date_to);
    case "get_comparison":
      return getComparison(args.period, args.date_from, args.date_to);
    case "get_pending":
      return getPending();
    case "get_tag_breakdown":
      return getTagBreakdown(args.period, args.date_from, args.date_to);
    ...
    case "get_payment_methods":
      return getPaymentMethods(args.period, args.date_from, args.date_to);
    ...
    case "search_contacts":
      return searchContacts(args.query, args.type, args.limit ?? 10);
    case "list_tags":
      return listTags();
    default:
...
// ============================================================
//  AGENTIC LOOP  — streams text + handles multi-step tool calls
// ============================================================
...
async function runAgenticLoop(
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    // tell client which tools are running
    for (const tc of toolCalls) {
      controller.enqueue(
        encoder.encode(
          `data: ${JSON.stringify({ type: "tool_call", name: tc.function.name })}\n\n`,
        ),
      );
    }
    ...
    // add tool results to history + notify client
    for (const r of results) {
      controller.enqueue(
        encoder.encode(
          `data: ${JSON.stringify({ type: "tool_result", name: r.name })}\n\n`,
        ),
      );
      stepMessages.push({
        role: "tool",
  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          await runAgenticLoop(fullMessages, controller, encoder);
        } catch (err: any) {
          controller.enqueue(
            encoder.encode(
