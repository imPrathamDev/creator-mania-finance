...
export async function getTimeSeries(
    ...
    { series: TimeSeriesPoint[]; granularity: Granularity; range: DateRange }
> {
    const range = resolveDateRange(period, custom);
...
export async function getTagBreakdown(
    ...
    const { data, error } = await supabase
        .from("transactions")
        .select(
            "type, amount, transaction_tags(tag:tags(id, name, color))",
        )
        .gte("transaction_date", range.from)
        .lte("transaction_date", range.to)
        .neq("payment_status", "cancelled");
    ...
    for (const r of data ?? []) {
        ...
        const tags: any[] = (r.transaction_tags ?? []).map((tt: any) => tt.tag)
            .filter(Boolean);
...
export async function getContactStats(
    ...
    return Array.from(map.values())
        .sort((a, b) =>
            (b.total_income + b.total_expense) -
...
export async function getIncomeExpenseRatio(
    ...
    return series.map((p) => ({
        ...
        expense: p.expense,
        ratio: p.expense > 0
            ? Math.round((p.income / p.expense) * 100) / 100
            : 0,
    }));
}
...
export async function getTopTransactions(
    ...
    let query = supabase
        .from("transactions")
        .select(
            "id, title, amount, type, transaction_date, contact:contacts(name)",
        )
        .gte("transaction_date", range.from)
        .lte("transaction_date", range.to)
        .neq("payment_status", "cancelled")
...
export async function getSavingsRate(
    ...
    return series.map((p) => ({
        ...
        expense: p.expense,
        savings: p.net,
        savings_rate: p.income > 0
            ? Math.round((p.net / p.income) * 10000) / 100
            : 0,
    }));
}
